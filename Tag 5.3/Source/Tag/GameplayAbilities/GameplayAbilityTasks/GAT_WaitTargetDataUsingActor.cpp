...
#include "GAT_WaitTargetDataUsingActor.h"
#include "AbilitySystemComponent.h"

UGAT_WaitTargetDataUsingActor::UGAT_WaitTargetDataUsingActor(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGAT_WaitTargetDataUsingActor* UGAT_WaitTargetDataUsingActor::WaitTargetDataWithReusableActor(UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType, AGameplayAbilityTargetActor* InTargetActor, bool bCreateKeyIfNotValidForMorePredicting)
{
	UGAT_WaitTargetDataUsingActor* MyObj = NewAbilityTask<UGAT_WaitTargetDataUsingActor>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
...
void UGAT_WaitTargetDataUsingActor::Activate()
{
	if (!IsValid(this))
	{
		return;
...
void UGAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback(const FGameplayAbilityTargetDataHandle& Data, FGameplayTag ActivationTag)
{
...
void UGAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback()
{
	check(AbilitySystemComponent.IsValid());
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UGAT_WaitTargetDataUsingActor::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent.IsValid());
	if (!Ability)
	{
...
void UGAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent.IsValid());
...
void UGAT_WaitTargetDataUsingActor::ExternalConfirm(bool bEndTask)
{
	check(AbilitySystemComponent.IsValid());
	if (TargetActor)
	{
...
void UGAT_WaitTargetDataUsingActor::ExternalCancel()
{
	check(AbilitySystemComponent.IsValid());
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UGAT_WaitTargetDataUsingActor::InitializeTargetActor() const
{
	check(TargetActor);
	check(Ability);
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(const_cast<UGAT_WaitTargetDataUsingActor*>(this), &ThisClass::OnTargetDataReadyCallback);
	TargetActor->CanceledDelegate.AddUObject(const_cast<UGAT_WaitTargetDataUsingActor*>(this), &ThisClass::OnTargetDataCancelledCallback);
}

void UGAT_WaitTargetDataUsingActor::FinalizeTargetActor() const
{
	check(TargetActor);
	check(Ability);
...
void UGAT_WaitTargetDataUsingActor::RegisterTargetDataCallbacks()
{
	ensure(IsValid(this));
	...
	if (!bIsLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &ThisClass::OnTargetDataReplicatedCancelledCallback);
...
void UGAT_WaitTargetDataUsingActor::OnDestroy(bool AbilityEnded)
{
	if (TargetActor)
	{
...
bool UGAT_WaitTargetDataUsingActor::ShouldReplicateDataToServer() const
{
	if (!Ability || !TargetActor)
	{
		return false;
